...
#define LOCTEXT_NAMESPACE "PROTOBUFLiveLinkSourceEditor"

void SPROTOBUFLiveLinkSourceFactory::Construct(const FArguments& Args)
{
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("PROTOBUFPortNumber", "Port Number"))
				]
				+ SHorizontalBox::Slot()
				.HAlign(HAlign_Fill)
				.FillWidth(0.5f)
				[
					...
					.OnTextCommitted(this, &SPROTOBUFLiveLinkSourceFactory::OnEndpointChanged)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Right)
			...
			[
				SNew(SButton)
				.OnClicked(this, &SPROTOBUFLiveLinkSourceFactory::OnOkClicked)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("Ok", "Ok"))
				]
...
void SPROTOBUFLiveLinkSourceFactory::OnEndpointChanged(const FText& NewValue, ETextCommit::Type)
{
...
FReply SPROTOBUFLiveLinkSourceFactory::OnOkClicked()
{
	TSharedPtr<SEditableTextBox> EditabledTextPin = EditabledText.Pin();
	if (EditabledTextPin.IsValid())
	{
